...
const SECTION_LABEL = 'Decoration Ideas';
const CATEGORY_FILTER = 'Decoration';
const VIEW_ALL_HREF = '/categories/decoration';
...
export default function DecorationSection() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        console.error('Error fetching decoration posts:', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (posts.length === 0) {
    return (
      <section className="container mx-auto px-4 py-12">
        <SectionHeader />
        <p className="text-center text-slate-400 py-12 text-sm">No decoration articles available yet.</p>
      </section>
    );
  }
    ...
    <section className="container mx-auto px-4 py-12">
      ...
      <div className="md:hidden">
        <div ref={carouselRef} className="grid grid-cols-1 gap-4 overflow-hidden"
          ...
          {firstCard && <BlogCardMobile post={firstCard} categoryLabel="Decoration" />}
          {secondCard && <div className="relative"><BlogCardMobile post={secondCard} categoryLabel="Decoration" /></div>}
        </div>
        {posts.length > 1 && (
