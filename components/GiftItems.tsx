...
const SECTION_LABEL = 'Gift Ideas';
const CATEGORY_FILTER = 'Gift Items';
const VIEW_ALL_HREF = '/categories/gift-items';
...
export default function GiftItems() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        console.error('Error fetching gift posts:', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (posts.length === 0) {
    return (
      <section className="container mx-auto px-4 py-12">
        <SectionHeader />
        <p className="text-center text-slate-400 py-12 text-sm">No gift ideas articles available yet.</p>
      </section>
    );
  }
    ...
    <section className="container mx-auto px-4 py-12">
      ...
      <div className="md:hidden">
        <div ref={carouselRef} className="grid grid-cols-1 gap-4 overflow-hidden"
          ...
          {firstCard && <BlogCardMobile post={firstCard} categoryLabel="Gift Ideas" />}
          {secondCard && <div className="relative"><BlogCardMobile post={secondCard} categoryLabel="Gift Ideas" /></div>}
        </div>
        {posts.length > 1 && (
